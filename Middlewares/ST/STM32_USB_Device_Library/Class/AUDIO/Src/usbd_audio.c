...
extern int32_t mainUsbBufferGetAvailableFrames(void);
...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  /* 07 byte*/
	...
	// Endpoint 2 - Standard Descriptor - Standard AS Isochronous Synch Endpoint Descriptor
	AUDIO_STANDARD_ENDPOINT_DESC_SIZE, 		/* bLength */
	USB_DESC_TYPE_ENDPOINT,            		/* bDescriptorType */
...
#define USB_OTG_HS_DEVICE ((USB_OTG_DeviceTypeDef *)((uint32_t)USB_OTG_HS + USB_OTG_DEVICE_BASE))
#endif
  ...
  if (haudio->iso_cont.tx_flag == 0U) {
    ...
    USB_OTG_INEndpointTypeDef *const ep = (USB_OTG_INEndpointTypeDef *)((uint32_t)USB_OTG_HS + USB_OTG_IN_ENDPOINT_BASE + (epnum * USB_OTG_EP_REG_SIZE));
...
static uint8_t USBD_AUDIO_IsoINIncomplete(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (haudio != NULL) {
  	haudio->iso_cont.fnsof = (USB_OTG_HS_DEVICE->DSTS & USB_OTG_DSTS_FNSOF) >> 8;
...
#define SAI_BUF_SIZE 192U
    ...
    haudio->iso_cont.ofs_packet = (haudio->iso_cont.ofs_packet + PacketSize) % SAI_BUF_SIZE;
    ...
    if ((haudio->iso_cont.usbintn % 4) == 0) {
			int32_t available_frames = mainUsbBufferGetAvailableFrames();
