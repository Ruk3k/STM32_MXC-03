...
void AutoWah::process(float32_t* pLeft, float32_t* pRight, uint32_t numSamples) {
	...
	if (cutoff > maxHz_) cutoff = maxHz_;

	updateFilter(cutoff);

	arm_biquad_cascade_df1_f32(&filterL_, pLeft, pLeft, numSamples);
	arm_biquad_cascade_df1_f32(&filterR_, pRight, pRight, numSamples);
	...
	for (uint32_t i = 0; i < numSamples; ++i) {
		...
		pRight[i] = ((dryR[i] * dryMixRate) + (pRight[i] * wetMixRate)) * makeUpGain_;
	}
}

void AutoWah::updateFilter(float32_t cutoffHz) {
	float32_t omega{ 2.0f * Constants::Math::kPi * cutoffHz / sampleRate_ };
