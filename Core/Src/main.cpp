...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
constexpr uint32_t kSamplesPerMs{ 48 };			// 48kHz / 1000ms = 48samples/ms
constexpr uint32_t kChannels{ 2 };			// Stereo(2ch)
constexpr uint32_t kLatencyMs{ 1 };			// 2 frames

// If you change kBufferSize, don't forget to change kMaxBufferSize in bass_preamp.hpp
constexpr uint32_t kBufferSize{ kSamplesPerMs * kChannels * kLatencyMs * 2 }; // Double buffering
/* USER CODE END PD */
...
int main(void)
{
  ...
  const auto bufferSize{ static_cast<uint16_t>(txBuffer.size()) };

  if (HAL_SAI_Transmit_DMA(&hsai_BlockB1, pTx, bufferSize) != HAL_OK) {
  	Error_Handler();
  }
  if (HAL_SAI_Receive_DMA(&hsai_BlockA1, pRx, bufferSize) != HAL_OK) {
  	Error_Handler();
  }
...
extern "C" {
	static void stereoToMono(uint32_t start, uint32_t end) {
	...
	static void handleAudioBlock(uint32_t start, uint32_t end) {
		stereoToMono(start, end);

		if (isBypassed) {
			std::copy(rxBuffer.begin() + start, rxBuffer.begin() + end, txBuffer.begin() + start);
		} else {
			uint32_t numSamples = end - start;
			sadowskyPreAmp.process(&txBuffer[start], &rxBuffer[start], numSamples);
		}
	}
